...
/** Save the full blocked-list for a user. */
async function dbSaveBlocked(userEmail, blockedIds) {
  await _db().collection('chat_blocked').doc(userEmail).set({ ids: blockedIds });
}

/** Load blocked emails for a user. Returns []. */
async function dbGetBlocked(userEmail) {
  const doc = await _db().collection('chat_blocked').doc(userEmail).get();
  return doc.exists ? (doc.data().ids || []) : [];
}

// ─── PHYSIO COMMUNICATION CENTER (student community feed) ────────────────────
// Collections:
//   community_posts/{postId}                      — feed posts (students + admin)
//   community_posts/{postId}/comments/{commentId}  — comments per post
//
// A post: {
//   id, authorEmail, authorName, authorRole ('student'|'admin'),
//   text, mentions:[email,...],
//   media:[{type:'image'|'video', data:base64dataURL, name?}],
//   link:{url,domain} | null,
//   pinned:boolean (admin/owner posts always pinned to top),
//   likes:[email,...], commentsCount:number, ts:number
// }

/** Save a new post (create or overwrite by id). */
async function dbSavePost(post) {
  await _db().collection('community_posts').doc(post.id).set(post, { merge: true });
}

/** Load all posts, newest first, pinned (admin/owner) posts always first. */
async function dbGetPosts() {
  const snap = await _db().collection('community_posts').get();
...
async function dbDeletePost(postId) {
  const commentsSnap = await _db().collection('community_posts').doc(postId).collection('comments').get();
  const batch = _db().batch();
...
/** Toggle a like on a post for a given user email. */
async function dbToggleLike(postId, email, liked) {
  const ref = _db().collection('community_posts').doc(postId);
  const fv = firebase.firestore.FieldValue;
  await ref.update({ likes: liked ? fv.arrayUnion(email) : fv.arrayRemove(email) });
}

/** Add a comment to a post. */
async function dbAddComment(postId, comment) {
  await _db().collection('community_posts').doc(postId)
    .collection('comments').doc(comment.id).set(comment);
  const fv = firebase.firestore.FieldValue;
  await _db().collection('community_posts').doc(postId).update({ commentsCount: fv.increment(1) }).catch(() => {});
}

/** Load all comments for a post, oldest first. */
async function dbGetComments(postId) {
  try {
    const snap = await _db().collection('community_posts').doc(postId)
      .collection('comments').orderBy('ts', 'asc').get();
    return snap.docs.map(d => d.data());
  } catch (e) {
    const snap = await _db().collection('community_posts').doc(postId).collection('comments').get();
    return snap.docs.map(d => d.data()).sort((a, b) => (a.ts || 0) - (b.ts || 0));
  }
}

/** Delete a single comment from a post. */
async function dbDeleteComment(postId, commentId) {
